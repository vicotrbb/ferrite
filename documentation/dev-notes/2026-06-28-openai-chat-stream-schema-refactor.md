# OpenAI Chat Stream Schema Refactor

Date: 2026-06-28

## Summary

Ferrite's OpenAI-compatible chat schema is split into smaller, focused modules.
Non-streaming chat request and response types remain in `chat.rs`, while chat
stream chunk/context types now live in `chat_stream.rs`.

## Implementation Notes

- Added `crates/ferrite-server/src/openai/schema/chat_stream.rs`.
- Kept public exports stable through `crates/ferrite-server/src/openai/schema.rs`.
- Moved only the existing chat streaming schema and context helpers.
- No JSON field names, object names, route behavior, or inference behavior
  changed.

The schema split reduced `chat.rs` from 336 lines to 207 lines. The new
`chat_stream.rs` module is 131 lines.
...
not add new OpenAI API features, change stream chunk serialization, or expand
real-model proof coverage.
