...
bounded one-token latency coverage for the same four endpoint shapes. The
SmolLM2-1.7B Q4_K_M server path now has six-prompt one-token legacy completion
and streaming legacy completion regressions. This proves the local server path
can drive real Tier 1 GGUF models for deterministic one-token responses, and
...
- Expand real Tier 1 HTTP server coverage beyond deterministic one-token
  ...
  Q8_0/Q6_K completions/chat, SmolLM2-1.7B Q4_K_M six-prompt legacy
  completions, and the bounded single-client latency and backpressure runs.
  ...
  queue-order probes, and Q8_0/Q6_K six-prompt legacy-completion probes, but
  server throughput, broader successful concurrent real-model serving, broader
  queue fairness, long-stream overlap, SmolLM2 streaming/chat HTTP behavior,
