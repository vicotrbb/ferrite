...
- Treat the Q4_K/Q6_K x Q8_K path as an approved opt-in kernel-contract
  ...
  no formula hole and the Qwen2.5-1.5B benchmark shows real throughput value,
  but SmolLM2-1.7B multi-token parity still fails, so the next slice must
  isolate the parity boundary or tighten the activation quantization strategy
  before promotion.
