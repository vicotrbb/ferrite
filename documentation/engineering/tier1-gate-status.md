...
tok/s and 3.69 tok/s on the default pool, also below target. A current-head
Qwen2.5-1.5B Q8_0 benchmark-token profile still measures about 6.44 tok/s and
continues to point at Q8_0 FFN roles plus the final output projection.
...
- Use `--profile-next-token` or `--profile-benchmark-token` to isolate hot
  ...
  gate/up, current-head local Qwen2.5-1.5B Q8_0 points at Q8_0 FFN
  gate/up/down plus output, local Qwen2.5-1.5B Q4_K_M points at Q4_K FFN plus
  Q6_K output/down roles, and x86_64 Qwen2.5-1.5B Q6_K now points to
