...
same OpenAI-compatible server paths now also have local three-request
...
legacy completion and chat completion regressions. The Qwen2.5-1.5B Q6_K
server path also has bounded one-token latency coverage for the same four
endpoint shapes plus six-prompt legacy-completion, non-streaming chat, and
streaming chat coverage. The
...
legacy-completion proofs plus Q8_0/Q6_K six-prompt chat proofs while
...
- Expand memory evidence beyond the bounded local one-token Qwen2.5-1.5B
  ...
  Q8_0/Q6_K sequential and three-request queue server RSS probes. Full-tier
  memory posture, large-context KV-cache growth, broader concurrent server
...
- Expand real Tier 1 HTTP server coverage beyond deterministic one-token
  ...
  Q8_0/Q6_K completions/chat, Qwen2.5-1.5B Q8_0 six-prompt legacy/chat
  completions, SmolLM2-1.7B Q4_K_M six-prompt legacy completions/chat in
  non-streaming and streaming forms, and the bounded
  ...
  queue-order probes, Q8_0/Q6_K six-prompt legacy-completion probes, and the
  Q8_0 six-prompt chat-completion probe, but
