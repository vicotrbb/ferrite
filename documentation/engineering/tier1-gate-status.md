...
The x86_64 long-chat proof has started with a bounded `staging` pod run for
Qwen2.5-0.5B Q4_K_M at the 256-token combined reconnect/error budget. That run
proves the same request-error recovery, disconnect/reconnect recovery, finish
reason, usage accounting, per-token latency summaries, and RSS sampling shape
on one amd64 AVX2 pod; the rest of the x86_64 token/model matrix remains
unproven.
...
| OpenAI-compatible x86_64 combined reconnect/error long-chat proof | Started for Qwen2.5-0.5B Q4_K_M at the 256 completion-token budget in a bounded amd64 AVX2 `staging` pod. This proves one x86_64 model/length shape only; the x86_64 512/1024 budgets, larger Tier 1 artifacts, longer steady-state behavior, and memory-focused reruns remain unproven | `documentation/benchmarks/2026-07-01-openai-long-chat-x86-qwen-0-5b-probe-256.md`; pod `ferrite-avx2-long-chat-qwen05-256` ran on `homelab-01` with CPU limit `2`, memory limit `6Gi`, and x86_64 AVX2 flags; the run completed four `finish_reason=length` scenarios with 256 completion tokens each, stable usage accounting, streaming timing, RSS samples, `401` error recovery, disconnect/reconnect completion, and `long_chat_summary_run_complete=true`; the pod was deleted after copying raw logs and the final cleanup check returned no pod |
