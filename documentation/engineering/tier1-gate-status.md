...
| Qwen2 Tier 1 model coverage | Config parsing started; runtime not supported yet | `documentation/dev-notes/2026-06-27-tier1-qwen2-0-5b-probe.md`; `documentation/dev-notes/2026-06-27-tier1-qwen2-config-parser.md`; Qwen2.5-0.5B-Instruct Q4_K_M downloaded successfully, synthetic `qwen2.*` config parsing now derives 7:1 GQA metadata, but the scalar runtime still exits at the explicit Llama-only loader boundary |
...
- Continue first-class Qwen2 architecture handling before claiming coverage for
  the Qwen2.5 Tier 1 models. Config parsing exists, but Qwen2.5-0.5B Q4_K_M
  runtime execution still stops at the explicit Llama-only loader boundary.
