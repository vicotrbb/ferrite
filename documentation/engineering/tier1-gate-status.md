...
| OpenAI-compatible model catalog endpoints | Proven for fixture raw HTTP and `async-openai` client model list/retrieve calls; dynamic multi-model catalogs, all OpenAI catalog semantics, real-model load behavior, and broader client ecosystem compatibility remain unproven | `documentation/dev-notes/2026-06-30-openai-model-catalog-proof.md`; `cargo test -p ferrite-server --test openai_client_catalog -- --nocapture` passed 2 tests covering `async-openai` model list and retrieve; `cargo test -p ferrite-server --test openai_http model -- --nocapture` passed 3 filtered raw HTTP tests covering `GET /v1/models`, `GET /v1/models/{model}`, and percent-encoded slash model ids |
