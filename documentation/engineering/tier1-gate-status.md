...
has the same x86_64 combined reconnect/error proof shape at the 256-token and
512-token budgets when using the known full-length operational prompt shape.
Those runs prove the same request-error recovery, disconnect/reconnect
recovery, finish reason, usage accounting, per-token latency summaries, and RSS
sampling shape on amd64 AVX2 pods. The Qwen2.5-0.5B 512/1024-token,
Qwen2.5-1.5B Q8_0, Qwen2.5-1.5B Q6_K, and SmolLM2-1.7B Q4_K_M 256-token runs
...
limit and is not proof that the 1024-token path fits 6Gi. The
SmolLM2-1.7B Q4_K_M 1024-token x86_64 budget
remains unproven.
