...
models. Tier 1 does not yet prove AVX2 runtime correctness, broad 0.5B-1.7B
...
| Next-token operation profiling | Proven for CLI and one real 1.7B profile | `documentation/dev-notes/2026-06-27-tier1-next-token-profile.md`; `--profile-next-token` emits per-operation labels; first SmolLM2-1.7B profile showed FFN roles as the largest aggregate matvec group |
...
- Use `--profile-next-token` to isolate hot operation labels before the next
  optimization slice. The first SmolLM2-1.7B profile points at FFN matvec roles
  and output projection rather than Q8_0/Q5_0 row scheduling.
