...
tok/s and 3.69 tok/s on the default pool, also below target.
...
- Use `--profile-next-token` or `--profile-benchmark-token` to isolate hot
  ...
  gate/up, local Qwen2.5-1.5B points at Q4_K FFN plus Q6_K output/down roles,
  and x86_64 Qwen2.5-1.5B Q6_K now points to transformer-layer Q6_K FFN roles
  after the output projection was improved with the AVX2 argmax route.
