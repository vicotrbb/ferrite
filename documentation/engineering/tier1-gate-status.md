...
the 256-token budget. That larger-artifact run proves the same request-error
and disconnect/reconnect behavior, finish reason, usage accounting, and timing
shape, but its RSS samples include anomalously low values and should not be
treated as memory-posture evidence.
...
| OpenAI-compatible combined reconnect/error long-chat proof | Proven locally for Qwen2.5-0.5B Q4_K_M at 256, 512, and 1024 completion tokens, and partially proven locally for Qwen2.5-1.5B Q8_0 at 256 completion tokens. Each run used `--probe-max-tokens`, so the request-error reconnect path, disconnect reconnect path, and four repeated streaming chat scenarios used the same token budget for each run. Larger Tier 1 artifacts, remaining larger-artifact token budgets, x86_64, longer steady-state behavior, and memory-focused reruns remain unproven | `documentation/benchmarks/2026-07-01-openai-long-chat-qwen-0-5b-probe-256.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-qwen-0-5b-probe-512.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-qwen-0-5b-probe-1024.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-qwen-1-5b-q8-probe-256.md`; Qwen2.5-0.5B Q4_K_M has completed 256/512/1024 runs with `long_chat_summary_run_complete=true`; Qwen2.5-1.5B Q8_0 has a 256-token run with probe max tokens of 256, four `finish_reason=length` scenarios with 43 prompt tokens, 256 completion tokens, 299 total tokens, streaming timing, and `long_chat_summary_run_complete=true`; all combined runs reported `401` error recovery, disconnect/reconnect completion, and `[DONE]`-terminated streaming responses; Qwen2.5-1.5B Q8_0 RSS samples include anomalously low values and are not memory-posture proof |
