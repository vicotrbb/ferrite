...
Q8_0 legacy completion and streaming legacy completion proofs. This proves the
...
one-token HTTP paths. It does not prove Tier 1 server throughput, Qwen2.5-1.5B
Q8_0 chat behavior, concurrent real-model successful serving, or broader Tier 1
...
- Expand real Tier 1 HTTP server coverage beyond deterministic one-token
  ...
  Q8_0 legacy completion/streaming completion, and the bounded single-client latency and
  backpressure runs. The configured bounded-wait path now has one real
  Qwen2.5-0.5B Q4_K_M overlap proof, but Qwen2.5-1.5B Q8_0 chat, server
