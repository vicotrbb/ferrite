...
| Qwen2 Tier 1 model coverage | Config and QKV bias prerequisites started; runtime not supported yet | `documentation/dev-notes/2026-06-27-tier1-qwen2-0-5b-probe.md`; `documentation/dev-notes/2026-06-27-tier1-qwen2-config-parser.md`; `documentation/dev-notes/2026-06-27-scalar-qkv-projection-bias.md`; Qwen2.5-0.5B-Instruct Q4_K_M downloaded successfully, synthetic `qwen2.*` config parsing now derives 7:1 GQA metadata, and scalar Q/K/V projection bias support is tested, but the scalar runtime still exits at the explicit Llama-only loader boundary |
...
- Continue first-class Qwen2 architecture handling before claiming coverage for
  the Qwen2.5 Tier 1 models. Config parsing and Q/K/V projection bias support
  exist, but Qwen2.5-0.5B Q4_K_M runtime execution still stops at the explicit
  Llama-only loader boundary.
