...
Qwen2.5-0.5B Q4_K_M now also has combined 256-token and 512-token full-length
long-chat proofs where the error reconnect request, disconnect reconnect
request, and four repeated streaming chat scenarios all use the same token
budget. Those runs report `finish_reason=length`, requested completion-token
counts on every scenario, client-side latency summaries, RSS samples, `401`
...
steady-state behavior, the 1024-token combined reconnect/error run, and
combined reconnect/error runs for the larger required Tier 1 artifacts remain
unproven.
...
| OpenAI-compatible combined reconnect/error long-chat proof | Partially proven locally for Qwen2.5-0.5B Q4_K_M at 256 and 512 completion tokens. Both runs used `--probe-max-tokens`, so the request-error reconnect path, disconnect reconnect path, and four repeated streaming chat scenarios used the same token budget for each run. This proves one local model and two full-length budgets only; the 1024-token combined run, larger Tier 1 artifacts, x86_64, longer steady-state behavior, and memory-focused reruns remain unproven | `documentation/benchmarks/2026-07-01-openai-long-chat-qwen-0-5b-probe-256.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-qwen-0-5b-probe-512.md`; the 256-token run reported probe max tokens of 256, four `finish_reason=length` scenarios with 43 prompt tokens, 256 completion tokens, 299 total tokens, streaming timing, RSS samples, and `long_chat_summary_run_complete=true`; the 512-token run reported probe max tokens of 512, four `finish_reason=length` scenarios with 43 prompt tokens, 512 completion tokens, 555 total tokens, streaming timing, RSS samples, and `long_chat_summary_run_complete=true`; both runs reported `401` error recovery, disconnect/reconnect completion, and `[DONE]`-terminated streaming responses |
