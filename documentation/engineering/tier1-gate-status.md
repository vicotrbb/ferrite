...
serving, or broader Tier 1 server behavior.
...
| OpenAI-compatible Tier 1 HTTP path | Proven for deterministic one-token Qwen2.5-0.5B Q4_K_M local server completions and chat, both non-streaming and SSE streaming; bounded single-client latency measured; concurrency and broader throughput remain unproven | `documentation/dev-notes/2026-06-28-openai-real-tier1-http-proof.md`; `documentation/dev-notes/2026-06-28-openai-real-tier1-http-streaming-proof.md`; `documentation/dev-notes/2026-06-28-openai-real-tier1-http-chat-proof.md`; `documentation/benchmarks/2026-06-28-tier1-qwen2-0-5b-http-latency.md`; `cargo test -p ferrite-server --test openai_real_tier1_http -- --ignored --nocapture`; explicit run passed 4 ignored real Tier 1 HTTP tests: `live_http_server_generates_with_real_tier1_model`, `live_http_server_streams_with_real_tier1_model`, `live_http_server_chats_with_real_tier1_model`, and `live_http_server_streams_chat_with_real_tier1_model`; bounded local HTTP averages for one-token Qwen2.5-0.5B Q4_K_M were about 240.944 ms for legacy completion, 573.722 ms for chat, 251.947 ms for legacy streaming, and 642.878 ms for chat streaming |
...
- Expand real Tier 1 HTTP server coverage beyond deterministic one-token
  ...
  run. Server throughput, concurrent real-model serving, and broader
  model/prompt behavior remain unproven.
