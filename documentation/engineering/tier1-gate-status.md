...
one-token CLI probes with both peak RSS and post-load current RSS samples, but
...
| Tier 1 memory posture | Bounded locally for one-token Qwen2.5-1.5B Q8_0 and Q6_K CLI probes with peak RSS and post-load current RSS samples; full-tier memory posture remains unproven | `documentation/benchmarks/2026-06-28-tier1-qwen2-1-5b-q8-q6-memory.md`; `documentation/dev-notes/2026-06-28-cli-load-memory-sampling-pause.md`; Qwen2.5-1.5B Q8_0 reported `model_file_bytes=1894532128`, `scalar_weight_bytes=1888581632`, `kv_cache_bytes=172032`, max RSS 3,821,076,480 bytes, peak footprint 3,822,526,784 bytes, and post-load current RSS 1,926,742,016 bytes; Qwen2.5-1.5B Q6_K reported `model_file_bytes=1464178720`, `scalar_weight_bytes=1458228224`, `kv_cache_bytes=172032`, max RSS 2,961,850,368 bytes, peak footprint 2,962,856,576 bytes, and post-load current RSS 1,497,677,824 bytes; both runs reported `model_file_retained_bytes=0` and matched expected generated token ID `198` |
...
- Expand memory evidence beyond the bounded local one-token Qwen2.5-1.5B
  ...
  server memory behavior, and longer-running steady-state RSS remain unproven.
