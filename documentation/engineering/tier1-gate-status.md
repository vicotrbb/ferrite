...
| OpenAI-compatible combined reconnect/error long-chat proof | Proven locally for Qwen2.5-0.5B Q4_K_M at 256, 512, and 1024 completion tokens. Each run used `--probe-max-tokens`, so the request-error reconnect path, disconnect reconnect path, and four repeated streaming chat scenarios used the same token budget for each run. This proves one local model only; larger Tier 1 artifacts, x86_64, longer steady-state behavior, and memory-focused reruns remain unproven | `documentation/benchmarks/2026-07-01-openai-long-chat-qwen-0-5b-probe-256.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-qwen-0-5b-probe-512.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-qwen-0-5b-probe-1024.md`; the 256-token run reported probe max tokens of 256, four `finish_reason=length` scenarios with 43 prompt tokens, 256 completion tokens, 299 total tokens, streaming timing, RSS samples, and `long_chat_summary_run_complete=true`; the 512-token run reported probe max tokens of 512, four `finish_reason=length` scenarios with 43 prompt tokens, 512 completion tokens, 555 total tokens, streaming timing, RSS samples, and `long_chat_summary_run_complete=true`; the 1024-token run reported probe max tokens of 1024, four `finish_reason=length` scenarios with 43 prompt tokens, 1024 completion tokens, 1067 total tokens, streaming timing, RSS samples, and `long_chat_summary_run_complete=true`; all three runs reported `401` error recovery, disconnect/reconnect completion, and `[DONE]`-terminated streaming responses |
