...
| OpenAI-compatible model catalog endpoints | Proven for fixture raw HTTP and `async-openai` client model list/retrieve calls, plus one loaded real Qwen2.5-0.5B Q4_K_M catalog probe; dynamic multi-model catalogs, all OpenAI catalog semantics, x86_64 server behavior, and broader client ecosystem compatibility remain unproven | `documentation/dev-notes/2026-06-30-openai-model-catalog-proof.md`; `documentation/dev-notes/2026-06-30-openai-real-model-catalog-proof.md`; `cargo test -p ferrite-server --test openai_client_catalog -- --nocapture` passed 2 tests covering `async-openai` model list and retrieve; `cargo test -p ferrite-server --test openai_http model -- --nocapture` passed 3 filtered raw HTTP tests covering `GET /v1/models`, `GET /v1/models/{model}`, and percent-encoded slash model ids; the real-model probe returned HTTP `200` for `/health`, `/v1/models`, and `/v1/models/qwen2.5-0.5b-q4_k_m-catalog-proof` with OpenAI-shaped model objects |
