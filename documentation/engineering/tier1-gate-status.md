...
Qwen2.5-0.5B Q4_K_M overlap proof while preserving the single-inference-permit
runtime invariant.
...
- Expand real Tier 1 HTTP server coverage beyond deterministic one-token
  ...
  overlap proof, but server throughput, broader successful concurrent
  real-model serving, queue fairness, and broader model/prompt behavior remain
  unproven.
