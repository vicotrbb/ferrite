...
chat completions, and streaming chat completions, plus an explicit
Qwen2.5-1.5B Q8_0 legacy completion proof. This proves the local server path can
drive real Tier 1 GGUF models for deterministic one-token responses, and a
bounded local latency benchmark now measures the Qwen2.5-0.5B one-token HTTP
paths. It does not prove Tier 1 server throughput, Qwen2.5-1.5B Q8_0 streaming
or chat behavior, concurrent real-model successful serving, or broader Tier 1
server behavior. A separate opt-in proof now verifies real Tier 1 backpressure:
...
- Expand real Tier 1 HTTP server coverage beyond deterministic one-token
  ...
  Q8_0 legacy completion, and the bounded single-client latency and
  backpressure runs. The configured bounded-wait path now has one real
  Qwen2.5-0.5B Q4_K_M overlap proof, but Qwen2.5-1.5B Q8_0 streaming/chat,
  server throughput, broader successful concurrent real-model serving, queue
  fairness, and broader model/prompt behavior remain unproven.
