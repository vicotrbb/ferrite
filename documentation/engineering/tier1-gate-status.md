...
Qwen2.5-0.5B Q4_K_M now also has a combined 256-token full-length
long-chat proof where the error reconnect request, disconnect reconnect
request, and four repeated streaming chat scenarios all use a 256-token budget.
That run reports `finish_reason=length`, 256 completion tokens on every
scenario, client-side latency summaries, RSS samples, `401` request-error
recovery, disconnect/reconnect recovery, and `long_chat_summary_run_complete`.
...
steady-state behavior, 512/1024-token combined reconnect/error runs, and
...
| OpenAI-compatible 256-token combined reconnect/error long-chat proof | Partially proven locally for Qwen2.5-0.5B Q4_K_M. The run used `--probe-max-tokens 256`, so the request-error reconnect path, disconnect reconnect path, and four repeated streaming chat scenarios used the same 256-token budget. This proves one local model and one full-length budget only; 512/1024-token combined runs, larger Tier 1 artifacts, x86_64, longer steady-state behavior, and memory-focused reruns remain unproven | `documentation/benchmarks/2026-07-01-openai-long-chat-qwen-0-5b-probe-256.md`; probe output reported `long_chat_error_probe_unauthorized_status=401`, `long_chat_error_probe_reconnect_completed=true`, `long_chat_error_probe_max_tokens=256`, `long_chat_disconnect_probe_aborted_after_generated_event=true`, `long_chat_disconnect_probe_reconnect_completed=true`, and `long_chat_disconnect_probe_max_tokens=256`; all four scenarios returned `finish_reason=length`, 43 prompt tokens, 256 completion tokens, 299 total tokens, streaming timing, RSS samples, and `long_chat_summary_run_complete=true` |
