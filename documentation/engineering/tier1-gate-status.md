...
SmolLM2-1.7B Q4_K_M server path now has a six-prompt one-token legacy
completion regression. This proves the local server path can drive real Tier 1
GGUF models for deterministic one-token responses, and bounded local latency
