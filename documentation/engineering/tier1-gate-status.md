...
Q4_K_M matched fixed local `llama.cpp` deterministic reference profiles for
four prompts. Qwen2.5-0.5B-Instruct Q8_0 also matched fixed local `llama.cpp`
deterministic reference profiles for the same four prompts. SmolLM2-1.7B,
Qwen2.5-0.5B, and Qwen2.5-1.5B also matched their
...
- Expand real model-output coverage beyond the fixed four-prompt profiles and
  current Q4_K_M plus Qwen2.5-0.5B Q8_0 quantization set.
...
- Expand Qwen2 coverage beyond the four fixed Q4_K_M prompt profiles and the
  local Qwen2.5-0.5B Q8_0 prompt profiles. Broader prompt coverage, 1.5B
  additional quantizations, x86_64 Q8_0 parity, and broader throughput remain
  unproven.
