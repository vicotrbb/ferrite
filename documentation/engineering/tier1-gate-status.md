...
| OpenAI-compatible EOS stop behavior | Proven locally for SmolLM2-1.7B Q4_K_M on the OpenAI-compatible legacy completion and streaming chat surfaces. The known EOS-sensitive prompt `The capital of France is` emitted tokenizer EOS as `<|im_end|>`, returned `finish_reason="stop"`, emitted `[DONE]` for streaming responses, and stopped before the requested token budget. Follow-up fixture regression coverage now suppresses EOS control text from assistant-visible OpenAI output while preserving token usage and `finish_reason="stop"`. Broader EOS behavior across the Qwen models, refreshed real-model polished EOS presentation, x86_64, and long-running serving remains unproven | `documentation/benchmarks/2026-06-30-openai-smollm-eos-stop-proof.md`; `documentation/dev-notes/2026-06-30-openai-eos-control-text-suppression.md`; streaming legacy completions returned 3 completion tokens for `max_tokens=6`; non-streaming legacy completions returned `finish_reason="stop"` with `Paris.<|im_end|>` in the raw pre-suppression proof; streaming chat completions returned 9 completion tokens for `max_tokens=16`, terminal `finish_reason="stop"`, usage, and one `[DONE]` marker; fixture regressions now assert EOS content is suppressed from visible completion output |
