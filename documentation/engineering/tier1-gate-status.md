...
proof milestone remains a dedicated long-chat gate closure that treats 256,
512, and 1024-token streaming responses, repeated multi-turn conversations, RSS
sampling before and after requests, latency per token, stop/EOS behavior, and
client reconnect/error behavior as one explicit contract. SmolLM2 now has
x86_64 EOS-specific evidence for one known prompt, but broader Qwen EOS
behavior, full-matrix EOS behavior across the required model set, longer
steady-state behavior, and memory-focused reruns remain unproven.
After the generated-context harness update, Qwen2.5-0.5B Q4_K_M now has one
fresh local 256-token long-chat rerun proving that turn 1 uses the seed
assistant context while turns 2-4 use generated assistant context from prior
completed streaming responses. That same run records token-limit status, fresh
...
| OpenAI-compatible generated-context long-chat proof | Partially proven locally for Qwen2.5-0.5B Q4_K_M at 256 completion tokens. This current-harness rerun proves the first turn used seed assistant context, follow-up turns used generated assistant context, token-limit status was present, reconnect after disconnect started a fresh generated stream, and the integrated run completed. This is one model and one token length only; 512/1024-token generated-context reruns, larger Tier 1 artifacts, x86_64 generated-context reruns, EOS-specific long-chat behavior, and steady-state serving remain unproven | `documentation/benchmarks/2026-07-01-openai-long-chat-qwen-0-5b-generated-context-probe-256.md`; the clean local release run on commit `3591b844224a606a52f58d1972e58fb5a143e1cb` wrote exit code `0`; turn 1 reported `long_chat_result_assistant_context_source=seed`, turns 2-4 reported `generated`; the summary reported `long_chat_summary_all_follow_up_turns_use_generated_context=true`, `long_chat_summary_all_token_limit_status_present=true`, `long_chat_summary_disconnect_probe_reconnect_started_new_generation=true`, and `long_chat_summary_run_complete=true` |
