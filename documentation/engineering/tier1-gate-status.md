...
The Qwen2.5 chat path now has bounded local release-build non-streaming chat
request-rate samples for Q4_K_M 0.5B and Q8_0 1.5B. Qwen2.5-0.5B Q4_K_M
...
concurrency 3 in 7,426 ms for about 1.21 req/s. This is narrow chat evidence
only; additional chat models, streaming throughput, longer generations,
x86_64, and steady-state behavior remain open.
The Qwen2.5-0.5B Q4_K_M path now also has bounded local release-build
streaming request-rate samples for both legacy completion SSE and chat SSE. The
legacy streaming path completed 10 sequential one-token requests in 1,716 ms
for about 5.83 req/s and 9 queued requests at client concurrency 3 in 1,818 ms
for about 4.95 req/s. The streaming chat path completed 10 sequential
one-token requests in 3,917 ms for about 2.55 req/s and 9 queued requests at
client concurrency 3 in 3,499 ms for about 2.57 req/s. This is one-model,
one-prompt streaming evidence only; additional streaming models, longer
