...
| OpenAI-compatible model catalog endpoints | Proven for fixture raw HTTP and `async-openai` client model list/retrieve calls, plus loaded real Qwen2.5-0.5B Q4_K_M catalog coverage by raw HTTP probe and repeatable raw HTTP and `async-openai` integration tests; real-model raw HTTP retrieval also covers a provider-style model id with a percent-encoded slash; dynamic multi-model catalogs, all OpenAI catalog semantics, x86_64 server behavior, and broader client ecosystem compatibility remain unproven | `documentation/dev-notes/2026-06-30-openai-model-catalog-proof.md`; `documentation/dev-notes/2026-06-30-openai-real-model-catalog-proof.md`; `crates/ferrite-server/tests/openai_client_real_tier1_catalog.rs`; `crates/ferrite-server/tests/openai_real_tier1_catalog.rs`; `cargo test -p ferrite-server --test openai_client_catalog -- --nocapture` passed 2 tests covering `async-openai` model list and retrieve; `cargo test -p ferrite-server --test openai_http model -- --nocapture` passed 3 filtered raw HTTP tests covering `GET /v1/models`, `GET /v1/models/{model}`, and percent-encoded slash model ids; `cargo test -p ferrite-server --test openai_client_real_tier1_catalog -- --ignored --nocapture` passed 1 real-model `async-openai` catalog test; `cargo test -p ferrite-server --test openai_real_tier1_catalog -- --ignored --nocapture` passed 1 real-model raw HTTP catalog test; `cargo test -p ferrite-server --test openai_real_tier1_catalog live_http_server_retrieves_real_tier1_provider_style_model_id -- --ignored --nocapture` passed 1 real-model encoded-slash raw HTTP catalog test; the real-model probe returned HTTP `200` for `/health`, `/v1/models`, and `/v1/models/qwen2.5-0.5b-q4_k_m-catalog-proof` with OpenAI-shaped model objects |
