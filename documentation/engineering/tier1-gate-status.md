...
combined reconnect/error budgets, plus Qwen2.5-1.5B Q6_K at the 256-token and
512-token combined reconnect/error budgets. Those runs prove the same
...
pods. The
512-token, 1024-token, Qwen2.5-1.5B Q8_0, and Qwen2.5-1.5B Q6_K runs also
encountered transient staging control-plane and node readiness or exec-stream
flaps, while the in-pod Ferrite gates still reached
...
| OpenAI-compatible x86_64 combined reconnect/error long-chat proof | Partially proven on bounded amd64 AVX2 `staging` pods. Qwen2.5-0.5B Q4_K_M has 256, 512, and 1024 completion-token x86_64 runs, and Qwen2.5-1.5B Q8_0 now has 256 and 512-token x86_64 runs. This proves those x86_64 model/length shapes only; Qwen2.5-1.5B Q8_0 1024, Qwen2.5-1.5B Q6_K, SmolLM2-1.7B Q4_K_M, longer steady-state behavior, and memory-focused reruns remain unproven | `documentation/benchmarks/2026-07-01-openai-long-chat-x86-qwen-0-5b-probe-256.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-x86-qwen-0-5b-probe-512.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-x86-qwen-0-5b-probe-1024.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-x86-qwen-1-5b-q8-probe-256.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-x86-qwen-1-5b-q8-probe-512.md`; all listed runs used bounded pods on `homelab-01` with CPU limit `2`, memory limit `6Gi`, and x86_64 AVX2 flags; the runs completed four `finish_reason=length` scenarios at their configured token budgets, stable usage accounting, streaming timing, RSS samples, `401` error recovery, disconnect/reconnect completion, and `long_chat_summary_run_complete=true`; the 512/1024 Qwen2.5-0.5B and Qwen2.5-1.5B Q8_0 runs observed staging control-plane or node-readiness flaps but the in-pod gates exited successfully; the Qwen2.5-1.5B Q8_0 512-token run peaked at `6335119360` bytes against a `6442450944` byte pod memory limit; each pod was deleted after copying raw logs |
