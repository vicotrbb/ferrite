...
| Next Q4_K/Q6_K kernel hypothesis | Proposed, not implemented | `documentation/research/2026-06-28-tier1-q4-q6-kernel-hypothesis.md`; the current Ferrite kernels dot GGUF Q4_K/Q6_K weights directly against `f32` activation vectors, while llama.cpp's relevant fast paths use intermediate `q8_K` activation blocks and int8 dot-style arithmetic; the next implementation slice should prefer a focused activation `q8_K` dot path over another row-scheduler experiment |
...
- Treat the next Q4_K/Q6_K throughput slice as a kernel-contract change, not a
  ...
  `q8_K` dot path before any repacked-weight layout work.
