...
The same Qwen2.5-0.5B Q4_K_M path now has a bounded local release-build
non-streaming chat request-rate sample: 10 sequential one-token chat requests
completed in 3,970 ms for about 2.52 req/s, and 9 queued chat requests at
client concurrency 3 completed in 3,603 ms for about 2.50 req/s. This is
single-model, single-prompt chat evidence only; additional chat models,
streaming throughput, longer generations, x86_64, and steady-state behavior
remain open.
