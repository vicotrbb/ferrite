...
single-inference-permit queue. Qwen2.5-0.5B Q4_K_M, Qwen2.5-1.5B Q8_0,
Qwen2.5-1.5B Q6_K, and SmolLM2-1.7B Q4_K_M now also have bounded three-client
streaming chat memory overlap samples.
...
The dedicated OpenAI-compatible long-chat gate is defined in
`documentation/engineering/tier1-openai-long-chat-gate.md`: 256, 512, and
1024-token streaming chat responses, repeated multi-turn conversations, RSS
sampling before and after requests, per-token latency, stop/EOS behavior, and
reconnect/error behavior. Ferrite now has local full-matrix evidence for all
four required Tier 1 HTTP model artifacts: Qwen2.5-0.5B Q4_K_M,
Qwen2.5-1.5B Q8_0, Qwen2.5-1.5B Q6_K, and SmolLM2-1.7B Q4_K_M. Those runs
prove the 256/512/1024 streaming chat lengths, repeated four-turn shape,
`finish_reason=length`, token usage accounting, client-side latency summaries,
and RSS samples for every required local model.
As separate partial evidence toward the same gate, all four required Tier 1
HTTP model artifacts now also have integrated stop-summary runs through the
...
Qwen2.5-0.5B Q4_K_M, Qwen2.5-1.5B Q8_0, Qwen2.5-1.5B Q6_K, and
SmolLM2-1.7B Q4_K_M also have bounded x86_64 AVX2 staging runs for
explicit-stop long-chat shapes, proving `finish_reason=stop`, emitted
streaming token events, usage accounting, timing summaries, RSS samples,
request-error recovery, disconnect/reconnect recovery, and
`long_chat_summary_run_complete=true` on amd64.
...
use the same token budget. Those runs report `finish_reason=length`, requested
completion-token counts on every scenario, client-side latency summaries, RSS
samples, `401` request-error recovery, disconnect/reconnect recovery, and
`long_chat_summary_run_complete`.
Qwen2.5-1.5B Q8_0 now has the same combined reconnect/error proof shape for
the 256-token, 512-token, and 1024-token budgets. Those larger-artifact runs
prove the same request-error and disconnect/reconnect behavior, finish reason,
usage accounting, and timing shape; the 256-token run's RSS samples include
anomalously low values and should not be treated as memory-posture evidence.
Qwen2.5-1.5B Q6_K now has the same combined reconnect/error proof shape for the
256-token, 512-token, and 1024-token budgets, including request-error recovery,
disconnect/reconnect recovery, finish reason, usage accounting, per-token
latency summaries, and RSS samples.
SmolLM2-1.7B Q4_K_M now has the same combined reconnect/error proof shape for
the 256-token, 512-token, and 1024-token budgets when using the known
full-length operational prompt shape from the earlier SmolLM2 full matrix.
After the generated-context harness update, Qwen2.5-0.5B Q4_K_M now has fresh
...
generated-context proof shape at 256, 512, and 1024 completion tokens. Those
runs record token-limit status, fresh disconnect reconnect generation, usage
accounting, timing, RSS samples, and `long_chat_summary_run_complete=true`.
Qwen2.5-1.5B Q6_K now has the same generated-context proof shape at 256, 512,
and 1024 completion tokens. SmolLM2-1.7B Q4_K_M now has the generated-context
proof shape at 256, 512, and 1024 completion tokens.
...
assistant context, RSS sampling before and after requests, latency per token,
...
remaining x86_64 generated-context matrix coverage, longer steady-state
behavior, and memory-focused reruns remain unproven.
After the shared-prefix prompt cache slice, SmolLM2-1.7B Q4_K_M also has a
local cached generated-context EOS gate for one answer-only prompt contract:
four OpenAI-compatible streaming chat turns, no explicit `stop` parameter,
natural `finish_reason=stop`, cached generated follow-up turns, token-id
coverage, RSS sampling, request-error recovery, disconnect/reconnect recovery,
and `long_chat_summary_run_complete=true`. A naive repeated completion prompt
cached generated follow-up turns but reached the token limit on turns 2-4, so
this is not broad EOS closure.
...
512-token, and 1024-token budgets when using the known full-length operational
prompt shape. Those runs prove the same request-error recovery,
disconnect/reconnect recovery, finish reason, usage accounting, per-token
latency summaries, and RSS sampling shape on amd64 AVX2 pods. The
Qwen2.5-0.5B 512/1024-token, Qwen2.5-1.5B Q8_0, Qwen2.5-1.5B Q6_K, and
SmolLM2-1.7B Q4_K_M 256-token runs also encountered transient staging
control-plane and node readiness or exec-stream flaps, while the in-pod Ferrite
gates still reached `long_chat_summary_run_complete=true`; they should not be
...
6Gi.
Qwen2.5-0.5B Q4_K_M now also has the first complete x86_64 generated-context
long-chat length slice at the 256-token, 512-token, and 1024-token budgets.
Those bounded amd64 AVX2 staging runs prove that turn 1 used seed assistant
context, turns 2-4 used generated assistant context, the request-error and
disconnect/reconnect probes completed, token-limit status was present, and
`long_chat_summary_run_complete=true`; larger required artifacts remain open.
Qwen2.5-1.5B Q6_K now has larger-artifact x86_64 generated-context slices at
the 256-token, 512-token, and 1024-token budgets, proving the same generated
...
integrated completion on amd64 AVX2 pods. Qwen2.5-1.5B Q8_0 now has
larger-artifact x86_64 generated-context slices at the 256-token, 512-token,
and 1024-token budgets. SmolLM2-1.7B Q4_K_M, broader EOS behavior, longer
steady-state serving, memory-focused reruns, and 6Gi fit for the 1024-token
Q6_K path or current Q8_0 pod shape remain open.
...
| --- | --- | --- |
| OpenAI-compatible full long-chat token matrix | Proven locally for the four required Tier 1 HTTP model artifacts. Each required model completed 12 streaming chat scenarios covering 256, 512, and 1024 completion tokens across four turns, with `finish_reason=length`, usage accounting, client-side latency summaries, and RSS samples. This proves the local full-length matrix shape; x86_64 behavior, longer steady-state behavior, full-matrix EOS behavior, and release-grade readiness remain unproven | `documentation/benchmarks/2026-06-30-openai-long-chat-qwen-0-5b-full-matrix.md`; `documentation/benchmarks/2026-06-30-openai-long-chat-qwen-1-5b-q6-full-matrix.md`; `documentation/benchmarks/2026-06-30-openai-long-chat-qwen-1-5b-q8-full-matrix.md`; `documentation/benchmarks/2026-06-30-openai-long-chat-smollm-1-7b-q4-full-matrix.md`; Qwen2.5-0.5B Q4_K_M, Qwen2.5-1.5B Q6_K, Qwen2.5-1.5B Q8_0, and SmolLM2-1.7B Q4_K_M all completed the required local 256/512/1024 matrix |
| OpenAI-compatible integrated long-chat stop summary | Partially proven locally for the four required Tier 1 HTTP model artifacts. Each run completed four repeated streaming chat stop scenarios with `finish_reason=stop`, usage accounting, client-side timing, RSS samples, request-level error recovery, disconnect/reconnect recovery, and `long_chat_summary_run_complete=true`. This is stop-focused short-run evidence only; 256/512/1024 stop-specific long-chat runs, broader EOS-specific evidence, x86_64 behavior, and longer steady-state behavior remain unproven | `documentation/benchmarks/2026-06-30-openai-long-chat-qwen-0-5b-integrated-stop-gate.md`; `documentation/benchmarks/2026-06-30-openai-long-chat-qwen-1-5b-q8-integrated-stop-gate.md`; `documentation/benchmarks/2026-06-30-openai-long-chat-qwen-1-5b-q6-integrated-stop-gate.md`; `documentation/benchmarks/2026-06-30-openai-long-chat-smollm-1-7b-integrated-stop-gate.md`; Qwen2.5-0.5B Q4_K_M, Qwen2.5-1.5B Q8_0, Qwen2.5-1.5B Q6_K, and SmolLM2-1.7B Q4_K_M all reported completed disconnect probes and `long_chat_summary_run_complete=true` |
| OpenAI-compatible combined reconnect/error long-chat proof | Proven locally for the four required Tier 1 HTTP model artifacts at 256, 512, and 1024 completion tokens. Each run used `--probe-max-tokens`, so the request-error reconnect path, disconnect reconnect path, and four repeated streaming chat scenarios used the same token budget for each run. Longer steady-state behavior and memory-focused reruns remain unproven; x86_64 combined reconnect/error coverage is tracked in the dedicated row below | `documentation/benchmarks/2026-07-01-openai-long-chat-qwen-0-5b-probe-256.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-qwen-0-5b-probe-512.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-qwen-0-5b-probe-1024.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-qwen-1-5b-q8-probe-256.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-qwen-1-5b-q8-probe-512.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-qwen-1-5b-q8-probe-1024.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-qwen-1-5b-q6-probe-256.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-qwen-1-5b-q6-probe-512.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-qwen-1-5b-q6-probe-1024.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-smollm-1-7b-q4-probe-256.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-smollm-1-7b-q4-probe-512.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-smollm-1-7b-q4-probe-1024.md`; Qwen2.5-0.5B Q4_K_M, Qwen2.5-1.5B Q8_0, Qwen2.5-1.5B Q6_K, and SmolLM2-1.7B Q4_K_M completed 256/512/1024 runs; each recorded matching probe token budgets, four `finish_reason=length` scenarios, stable usage accounting, streaming timing, and `long_chat_summary_run_complete=true`; all combined runs reported `401` error recovery, disconnect/reconnect completion, and `[DONE]`-terminated streaming responses; the Qwen2.5-1.5B Q8_0 256-token RSS samples include anomalously low values and are not memory-posture proof; the SmolLM2 1024-token run showed increasing RSS across measured turns and is not leak-freedom proof |
| OpenAI-compatible generated-context long-chat proof | Proven locally for the four required Tier 1 HTTP model artifacts at 256, 512, and 1024 completion tokens. These current-harness reruns prove the first turn used seed assistant context, follow-up turns used generated assistant context, token-limit status was present, reconnect after disconnect started a fresh generated stream, and each integrated run completed. The x86_64 generated-context proof now covers Qwen2.5-0.5B Q4_K_M at 256, 512, and 1024 completion tokens, Qwen2.5-1.5B Q6_K at 256, 512, and 1024 completion tokens, and Qwen2.5-1.5B Q8_0 at 256, 512, and 1024 completion tokens, but the remaining SmolLM2-1.7B x86_64 generated-context coverage, EOS-specific long-chat behavior, and steady-state serving remain incomplete | `documentation/benchmarks/2026-07-01-openai-long-chat-qwen-0-5b-generated-context-probe-256.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-qwen-0-5b-generated-context-probe-512.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-qwen-0-5b-generated-context-probe-1024.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-qwen-1-5b-q8-generated-context-probe-256.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-qwen-1-5b-q8-generated-context-probe-512.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-qwen-1-5b-q8-generated-context-probe-1024.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-qwen-1-5b-q6-generated-context-probe-256.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-qwen-1-5b-q6-generated-context-probe-512.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-qwen-1-5b-q6-generated-context-probe-1024.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-smollm-1-7b-q4-generated-context-probe-256.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-smollm-1-7b-q4-generated-context-probe-512.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-smollm-1-7b-q4-generated-context-probe-1024.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-x86-qwen-0-5b-generated-context-probe-256.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-x86-qwen-0-5b-generated-context-probe-512.md`; `documentation/benchmarks/2026-07-02-openai-long-chat-x86-qwen-0-5b-generated-context-probe-1024.md`; `documentation/benchmarks/2026-07-02-openai-long-chat-x86-qwen-1-5b-q6-generated-context-probe-256.md`; `documentation/benchmarks/2026-07-02-openai-long-chat-x86-qwen-1-5b-q6-generated-context-probe-512.md`; `documentation/benchmarks/2026-07-02-openai-long-chat-x86-qwen-1-5b-q6-generated-context-probe-1024.md`; `documentation/benchmarks/2026-07-02-openai-long-chat-x86-qwen-1-5b-q8-generated-context-probe-256.md`; `documentation/benchmarks/2026-07-02-openai-long-chat-x86-qwen-1-5b-q8-generated-context-probe-512.md`; `documentation/benchmarks/2026-07-02-openai-long-chat-x86-qwen-1-5b-q8-generated-context-probe-1024.md`; the Qwen2.5-0.5B, Qwen2.5-1.5B Q8_0, Qwen2.5-1.5B Q6_K, and SmolLM2-1.7B Q4_K_M local release runs at 256, 512, and 1024 tokens all wrote exit code `0`; all listed local runs reported turn 1 as `long_chat_result_assistant_context_source=seed`, turns 2-4 as `generated`, and summaries with `long_chat_summary_all_follow_up_turns_use_generated_context=true`, `long_chat_summary_all_token_limit_status_present=true`, `long_chat_summary_disconnect_probe_reconnect_started_new_generation=true`, and `long_chat_summary_run_complete=true`; the x86_64 Qwen2.5-0.5B 256-token, 512-token, and 1024-token runs, x86_64 Qwen2.5-1.5B Q6_K 256-token, 512-token, and 1024-token runs, and x86_64 Qwen2.5-1.5B Q8_0 256-token, 512-token, and 1024-token runs wrote exit code `0`, reported turn 1 as seed, turns 2-4 as generated, requested completion tokens for each turn, `finish_reason=length`, `long_chat_summary_all_follow_up_turns_use_generated_context=true`, and `long_chat_summary_run_complete=true`; the x86_64 Qwen2.5-1.5B Q6_K 1024-token run and Qwen2.5-1.5B Q8_0 256-token, 512-token, and 1024-token runs used 8Gi memory limits, so they are not evidence that those paths fit the earlier 6Gi pod limit |
| OpenAI-compatible x86_64 combined reconnect/error long-chat proof | Proven on bounded amd64 AVX2 `staging` pods for the four required Tier 1 HTTP model artifacts at 256, 512, and 1024 completion-token budgets. This proves the required x86_64 combined reconnect/error model/length matrix only; broader EOS-specific behavior, longer steady-state behavior, and memory-focused reruns remain unproven | `documentation/benchmarks/2026-07-01-openai-long-chat-x86-qwen-0-5b-probe-256.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-x86-qwen-0-5b-probe-512.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-x86-qwen-0-5b-probe-1024.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-x86-qwen-1-5b-q8-probe-256.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-x86-qwen-1-5b-q8-probe-512.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-x86-qwen-1-5b-q8-probe-1024.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-x86-qwen-1-5b-q6-probe-256.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-x86-qwen-1-5b-q6-probe-512.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-x86-qwen-1-5b-q6-probe-1024.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-x86-smollm-1-7b-q4-probe-256.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-x86-smollm-1-7b-q4-probe-512.md`; `documentation/benchmarks/2026-07-01-openai-long-chat-x86-smollm-1-7b-q4-probe-1024.md`; all listed runs used bounded pods on `homelab-01` with x86_64 AVX2 flags; the runs completed four `finish_reason=length` scenarios at their configured token budgets, stable usage accounting, streaming timing, RSS samples, `401` error recovery, disconnect/reconnect completion, and `long_chat_summary_run_complete=true`; several runs observed staging control-plane or node-readiness flaps but the in-pod gates exited successfully; the Qwen2.5-1.5B Q8_0 512-token run peaked at `6335119360` bytes against a `6442450944` byte pod memory limit; each pod was deleted after copying raw logs |
| OpenAI-compatible EOS stop behavior | Proven locally and on a bounded x86_64 `staging` pod for SmolLM2-1.7B Q4_K_M on the OpenAI-compatible legacy completion and streaming chat surfaces. The known EOS-sensitive prompt `The capital of France is` terminates through tokenizer EOS, returns `finish_reason="stop"`, emits `[DONE]` exactly once for streaming responses, stops before the requested token budget, preserves usage accounting, and no longer exposes `<|im_end|>` as assistant-visible OpenAI output after the suppression fix. SmolLM2-1.7B Q4_K_M also has a local cached generated-context EOS gate for one answer-only long-chat prompt contract with no explicit `stop` parameter. Broader EOS behavior across the Qwen models, the full long-chat EOS matrix, x86_64 cached generated-context EOS behavior, longer steady-state serving, and memory-focused reruns remain unproven | `documentation/benchmarks/2026-06-30-openai-smollm-eos-stop-proof.md`; `documentation/benchmarks/2026-07-01-openai-x86-smollm-1-7b-q4-eos-stop-proof.md`; `documentation/dev-notes/2026-06-30-openai-eos-control-text-suppression.md`; `documentation/benchmarks/2026-07-02-openai-long-chat-smollm-1-7b-shared-prefix-cache-eos-gate.md`; pre-suppression streaming legacy completions returned 3 completion tokens for `max_tokens=6` and mapped EOS to terminal `finish_reason="stop"`; refreshed suppression rerun returned streaming legacy text ` Paris` and `.`, terminal `finish_reason="stop"`, usage with 5 prompt tokens, 3 completion tokens, 8 total tokens, and `[DONE]`, with no visible `<|im_end|>`; refreshed non-streaming legacy completions returned `text:" Paris."`, `finish_reason="stop"`, and the same usage; refreshed streaming chat completions returned visible content through ` Paris.`, terminal `finish_reason="stop"`, usage with 12 prompt tokens, 9 completion tokens, 21 total tokens, and `[DONE]`, with no visible `<|im_end|>`; the x86_64 rerun used bounded pod `ferrite-avx2-eos-smollm17-q4`, model SHA256 `77665ea4815999596525c636fbeb56ba8b080b46ae85efef4f0d986a139834d7`, and reported non-streaming completion text ` Paris.`, streaming completion text ` Paris` plus `.`, streaming chat content through ` Paris.`, terminal `finish_reason="stop"`, one `[DONE]` marker for each streaming response, completion usage `3` for legacy completions and `9` for streaming chat, and `visible_eos_marker_present=false`; the cached generated-context EOS gate completed four streaming chat turns with natural `finish_reason=stop`, cached generated follow-up turns, streaming token IDs, RSS sampling, request-error recovery, disconnect/reconnect recovery, and `long_chat_summary_run_complete=true`, while the naive repeated completion prompt cached follow-up turns but hit `finish_reason=length` on turns 2-4 |
...
| OpenAI-compatible SmolLM2-1.7B Q4_K_M three-client streaming chat memory | Bounded locally for three overlapping streaming `POST /v1/chat/completions` clients with 32 completion tokens each and a configured 60s wait window; broad high concurrency, queue fairness beyond this three-client shape, leak freedom, x86_64 behavior, and broader prompts remain unproven | `documentation/benchmarks/2026-06-30-tier1-smollm2-1-7b-q4-http-chat-stream-three-client-memory.md`; SmolLM2-1.7B Q4_K_M sampled 433,127,424 bytes after health, peaked at 1,483,489,280 bytes during the three-client probe, sampled 1,096,941,568 bytes after all clients completed, and sampled 1,096,941,568 bytes after a two-second idle sample; all three clients returned HTTP `200`, emitted `[DONE]`, produced 35 SSE event chunks, and included usage with 9 prompt tokens, 32 completion tokens, and 41 total tokens; observed finish order was `first_then_second_then_third` |
| OpenAI-compatible Qwen2.5-0.5B Q4_K_M three-client streaming chat memory | Bounded locally for three overlapping streaming `POST /v1/chat/completions` clients with 32 completion tokens each and a configured 60s wait window; broad high concurrency, queue fairness beyond this three-client shape, leak freedom, x86_64 behavior, and broader prompts remain unproven | `documentation/benchmarks/2026-06-30-tier1-qwen2-0-5b-q4-http-chat-stream-three-client-memory.md`; Qwen2.5-0.5B Q4_K_M sampled 431,046,656 bytes after health, peaked at 459,620,352 bytes during the three-client probe, sampled 459,620,352 bytes after all clients completed, and sampled 459,620,352 bytes after a two-second idle sample; all three clients returned HTTP `200`, emitted `[DONE]`, produced 35 SSE event chunks, and included usage with 8 prompt tokens, 32 completion tokens, and 40 total tokens; observed finish order was `first_then_second_then_third` |
| OpenAI-compatible model catalog endpoints | Proven for fixture raw HTTP and `async-openai` client model list/retrieve calls, plus loaded real Qwen2.5-0.5B Q4_K_M catalog coverage by raw HTTP probe and repeatable raw HTTP and `async-openai` integration tests; real-model raw HTTP retrieval covers a provider-style model id with a percent-encoded slash, real-model `async-openai` retrieval covers the same provider-style shape when the client sends literal slash path segments, and provider-style literal and encoded model paths have CORS preflight coverage; dynamic multi-model catalogs, all OpenAI catalog semantics, x86_64 server behavior, and broader client ecosystem compatibility remain unproven | `documentation/dev-notes/2026-06-30-openai-model-catalog-proof.md`; `documentation/dev-notes/2026-06-30-openai-real-model-catalog-proof.md`; `documentation/dev-notes/2026-06-30-openai-provider-model-cors-proof.md`; `crates/ferrite-server/src/openai/catalog_tests.rs`; `crates/ferrite-server/src/openai/auth_tests.rs`; `crates/ferrite-server/tests/openai_client_real_tier1_catalog.rs`; `crates/ferrite-server/tests/openai_real_tier1_catalog.rs`; `cargo test -p ferrite-server --test openai_client_catalog -- --nocapture` passed 2 tests covering `async-openai` model list and retrieve; `cargo test -p ferrite-server --test openai_http model -- --nocapture` passed 3 filtered raw HTTP tests covering `GET /v1/models`, `GET /v1/models/{model}`, and percent-encoded slash model ids; `cargo test -p ferrite-server --test openai_client_real_tier1_catalog -- --ignored --nocapture` passed 2 real-model `async-openai` catalog tests, including provider-style literal-slash retrieval; `cargo test -p ferrite-server --test openai_real_tier1_catalog -- --ignored --nocapture` passed 2 real-model raw HTTP catalog tests, including provider-style encoded-slash retrieval; `cargo test -p ferrite-server --lib model_retrieve_endpoint_supports_literal_slash_model_ids_from_clients -- --nocapture`, `cargo test -p ferrite-server --lib model_retrieve_endpoint_supports_encoded_slashes -- --nocapture`, `cargo test -p ferrite-server --lib model_retrieve_endpoint_rejects_unknown_model -- --nocapture`, and `cargo test -p ferrite-server --lib openai_model_retrieve_preflight_supports_provider_style_model_ids -- --nocapture` each passed; the real-model probe returned HTTP `200` for `/health`, `/v1/models`, and `/v1/models/qwen2.5-0.5b-q4_k_m-catalog-proof` with OpenAI-shaped model objects |
...
| OpenAI-compatible Qwen2.5-1.5B Q8_0 three-client streaming chat memory | Bounded locally for three overlapping streaming `POST /v1/chat/completions` clients with 32 completion tokens each and a configured 60s wait window; broad high concurrency, queue fairness beyond this three-client shape, leak freedom, x86_64 behavior, and broader prompts remain unproven | `documentation/benchmarks/2026-06-30-tier1-qwen2-1-5b-q8-http-chat-stream-three-client-memory.md`; Qwen2.5-1.5B Q8_0 sampled 179,027,968 bytes after health, peaked at 1,693,908,992 bytes during the three-client probe, sampled 1,693,614,080 bytes after all clients completed, and sampled 1,693,614,080 bytes after a two-second idle sample; all three clients returned HTTP `200`, emitted `[DONE]`, produced 33 SSE event chunks, and included usage with 8 prompt tokens, 32 completion tokens, and 40 total tokens; observed finish order was `first_then_second_then_third` |
