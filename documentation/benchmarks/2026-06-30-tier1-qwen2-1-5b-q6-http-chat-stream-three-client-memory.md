# Tier 1 Qwen2.5-1.5B Q6_K HTTP Chat Streaming Three-Client Memory
...
for three overlapping Qwen2.5-1.5B-Instruct Q6_K 32-token streaming
...
extends the existing two-client Qwen2.5-1.5B Q6_K concurrent streaming memory
...
- Commit before documentation: `3c46252`
...
- Server model ID: `qwen2.5-1.5b-q6_k-chatstream32-three-client`
...
target/release/ferrite-server \
  ...
  --model-id qwen2.5-1.5b-q6_k-chatstream32-three-client \
...
```text
{"status":"ok","ready":true,"model":"qwen2.5-1.5b-q6_k-chatstream32-three-client"}
```
...
- Client start spacing: second client started 0.051525 seconds after first
- Client start spacing: third client started 0.104704 seconds after first
...
```json
{"model":"qwen2.5-1.5b-q6_k-chatstream32-three-client","messages":[{"role":"user","content":"hello world"}],"max_completion_tokens":32,"stream":true,"stream_options":{"include_usage":true}}
```
...
| --- | ---: | ---: |
| After health | 951,200 | 974,028,800 |
| Maximum during three-client probe | 2,378,608 | 2,435,694,592 |
| After all clients completed | 1,480,608 | 1,516,142,592 |
| Two seconds idle after all clients completed | 1,480,608 | 1,516,142,592 |

The max RSS sample occurred 29.910784 seconds after probe start. The probe took
34.279718 seconds wall-clock and collected 131 RSS samples.
...
| --- | ---: | ---: | ---: | ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| First streaming chat client | 200 | 11.749402 s | 9,787 | 34 | true | `length` | `assistant` | 77 | 8 | 32 | 40 |
| Second streaming chat client | 200 | 23.444620 s | 9,787 | 34 | true | `length` | `assistant` | 77 | 8 | 32 | 40 |
| Third streaming chat client | 200 | 34.137944 s | 9,787 | 34 | true | `length` | `assistant` | 77 | 8 | 32 | 40 |
...
After the benchmark, `lsof -nP -iTCP:18101 -sTCP:LISTEN` returned no listener.

## Interpretation

The three-client streaming chat probe peaked at about 2.44 GB RSS while clients
were active and settled at about 1.52 GB after completion and a two-second idle
...
memory gap for Qwen2.5-1.5B Q6_K. It does not prove broad high concurrency,
