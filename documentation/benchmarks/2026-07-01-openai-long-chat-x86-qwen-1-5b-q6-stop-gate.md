# OpenAI Long-Chat x86_64 Qwen 1.5B Q6 Stop Gate

## Scope

This run adds x86_64 stop-focused evidence for
`Qwen2.5-1.5B-Instruct-Q6_K`. It exercises Ferrite's OpenAI-compatible
...
- Date: 2026-07-01
- Commit: `3b086ec355d639c633bf6fcc82b98675694b3f0c`
- Kubernetes context: `staging`
- Pod: `ferrite-avx2-stop-qwen15-q6`
- Node: `homelab-01`
...
- Server port inside pod: `127.0.0.1:18132`
- Release build cgroup memory peak: `2119864320` bytes
- Post-health cgroup memory peak: `4646907904` bytes
- Pod cgroup memory peak after proof: `4646907904` bytes
- Workspace size after source copy, model copy, release build, and proof:
  `1.6G`
- Raw probe log:
  `target/proof/x86-qwen-1-5b-q6-long-chat-stop-probe.log`
- Raw server log:
  `target/proof/x86-qwen-1-5b-q6-long-chat-stop.log`
...
kind: Pod
metadata:
  name: ferrite-avx2-stop-qwen15-q6
spec:
  restartPolicy: Never
...
```sh
kubectl --context staging exec ferrite-avx2-stop-qwen15-q6 -- sh -lc \
...
```text
Finished `release` profile [optimized] target(s) in 37.07s
```
...
```text
PID 1658, RSS 1454956 KiB
memory_current=3185426432
memory_peak=4646907904
```
...
`target/proof/x86-qwen-1-5b-q6-long-chat-stop-probe.exit`.
...
| --- | ---: | ---: | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 1 | 1 | 1 | stop | 22301 | 1 | 20280 | 20280 | 0.049309 | 20280 | 20280 | 20280 | 20280 | 1504423936 | 1504145408 | 1504145408 |
| 2 | 1 | 1 | stop | 22163 | 1 | 20151 | 20151 | 0.049624 | 20151 | 20151 | 20151 | 20151 | 1504145408 | 1504497664 | 1504497664 |
| 3 | 1 | 1 | stop | 22278 | 1 | 20265 | 20265 | 0.049345 | 20265 | 20265 | 20265 | 20265 | 1504497664 | 1504608256 | 1504608256 |
| 4 | 1 | 1 | stop | 22178 | 1 | 20165 | 20165 | 0.049589 | 20165 | 20165 | 20165 | 20165 | 1504608256 | 1504493568 | 1504493568 |
...
```sh
kubectl --context staging delete pod ferrite-avx2-stop-qwen15-q6 --wait=true --timeout=120s
kubectl --context staging get pod ferrite-avx2-stop-qwen15-q6 --ignore-not-found
```

The final `get pod` command returned no output. A transient `apiserver not
ready` watch warning appeared during cleanup verification, and a follow-up node
check showed both `homelab-01` and `homelab-02` as `Ready`.

## Interpretation

Ferrite now has a real x86_64 AVX2 long-chat stop proof for Qwen2.5-1.5B Q6_K.
The run proves the OpenAI-compatible streaming chat path can finish through an
explicit stop sequence with repeated four-turn shape, request-error recovery,
disconnect/reconnect recovery, timing summaries, RSS samples, and
`long_chat_summary_run_complete=true`.

Remaining proof gaps:

- repeat the x86_64 stop-focused long-chat run for SmolLM2-1.7B Q4_K_M;
