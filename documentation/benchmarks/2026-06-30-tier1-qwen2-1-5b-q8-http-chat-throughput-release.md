# Tier 1 Qwen2.5-1.5B Q8_0 Release HTTP Chat Throughput
...
elapsed_ms=7426
requests_per_second=1.211883
```

After the benchmark, `lsof -nP -iTCP:18085 -sTCP:LISTEN` returned no listener.

## Interpretation

The release OpenAI-compatible non-streaming chat path can serve bounded
one-token Qwen2.5-1.5B Q8_0 chat requests at about 1.19 requests per second on
this local Apple M1 Pro host. The queued three-client shape measured about
1.21 requests per second because model execution remains serialized by the
single inference permit.

This adds release binary request-rate evidence for a larger Tier 1 Qwen chat
completion shape. Broader HTTP throughput remains open: Q6_K and SmolLM2 chat
models, streaming, longer generations, multiple prompts, x86_64, longer
