...
pub struct LongChatGateConfig {
    ...
    required_finish_sources: Vec<String>,
...
impl LongChatGateConfig {
    pub fn parse(args: impl IntoIterator<Item = OsString>) -> Result<Self, LongChatGateError> {
        ...
        while let Some(arg) = iter.next() {
            ...
            match flag {
                ...
                "--require-finish-sources" => {
                    config.required_finish_sources = parse_non_empty_list(
                        next_value(&mut iter, "--require-finish-sources")?,
                        "--require-finish-sources",
                    )?;
                }
                "--probe-max-tokens" => {
    ...
    pub fn required_finish_sources(&self) -> &[String] {
        &self.required_finish_sources
    }
...
impl Default for LongChatGateConfig {
    fn default() -> Self {
        Self {
            ...
            required_finish_sources: Vec::new(),
