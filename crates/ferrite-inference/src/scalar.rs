...
fn dot(left: &[f32], right: &[f32]) -> Result<f32, InferenceError> {
    ensure_len("dot right", right, left.len())?;
    Ok(left.iter().zip(right.iter()).map(|(a, b)| a * b).sum())
}

fn add_assign(left: &mut [f32], right: &[f32]) -> Result<(), InferenceError> {
...
fn softmax(values: &[f32]) -> Result<Vec<f32>, InferenceError> {
    if values.is_empty() {
...
fn ensure_len(name: &str, values: &[f32], expected: usize) -> Result<(), InferenceError> {
