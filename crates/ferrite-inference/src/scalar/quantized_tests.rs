...
#[test]
#[cfg(target_arch = "aarch64")]
fn q8_0_large_moderate_width_matvec_uses_neon_row_parallel() -> Result<(), InferenceError> {
    let rows = 4096;
    let cols = 32;
    let mut bytes = Vec::new();
    for row in 0..rows {
        bytes.extend(q8_0_block_with_value(if row % 2 == 0 { 1 } else { -2 }));
    }

    let output = q8_0_mul_vec_with_backend(&bytes, rows, cols, &[1.0; 32])?;
