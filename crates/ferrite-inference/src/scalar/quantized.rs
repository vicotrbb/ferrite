...
mod tests {
    ...
    fn q5_0_matvec_uses_neon_backend_on_aarch64() -> Result<(), InferenceError> {
        ...
        let output = q5_0_mul_vec_with_backend(&bytes, 1, 32, &[1.0; 32])?;
    ...
    #[test]
    #[cfg(any(target_arch = "aarch64", target_arch = "x86_64"))]
    fn q5_0_simd_matvec_uses_row_parallel_backend() -> Result<(), InferenceError> {
        let mut bytes = Vec::new();
        bytes.extend(q5_0_block_with_value(1));
        bytes.extend(q5_0_block_with_value(2));
        bytes.extend(q5_0_block_with_value(-3));

        let output = q5_0_mul_vec_with_backend(&bytes, 3, 32, &[1.0; 32])?;

        #[cfg(target_arch = "aarch64")]
        assert_eq!(output.backend, Q5_0MatVecBackend::Aarch64NeonRowParallel);
        #[cfg(target_arch = "x86_64")]
        assert_eq!(output.backend, Q5_0MatVecBackend::X86_64Avx2RowParallel);
