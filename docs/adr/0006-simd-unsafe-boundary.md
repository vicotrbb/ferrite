...
At the time of acceptance, this ADR defined the conditions for the first unsafe
SIMD implementation. The current repository now contains scoped AVX2, NEON,
DotProd, and I8MM modules that follow this boundary.
...
- [`0003-scalar-reference-inference-boundary.md`](0003-scalar-reference-inference-boundary.md)
  establishes the scalar correctness oracle.
- [`0005-reference-parity-policy.md`](0005-reference-parity-policy.md) defines
  output claims and token-trace evidence.
- `crates/ferrite-inference/tests/matvec_kernel_check.rs` compares optimized
  matrix operations with decoded scalar references.
- Architecture-specific modules carry `unsafe_code` allowances with reasons,
  explicit unsafe blocks, and local safety comments enforced by Clippy.
- [`../safety.md`](../safety.md) defines the current review requirements.
